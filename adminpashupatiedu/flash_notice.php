...
// Close the database connection outside the try-catch block
...
if (isset($_POST['update_flash_notice'])) {
    ...
    // Move uploaded file if there is a new one
    ...
    // Prepare and execute the update query using the correct connection variable
    $sql = "UPDATE flash_notice SET title=?, message=?, trun_flash=?, image_url=? WHERE id=1";
    if ($stmt = mysqli_prepare($connection, $sql)) {
        mysqli_stmt_bind_param($stmt, "ssss", $flash_title, $flash_message, $trun_flash_notice, $sqlfileurl);

        if (mysqli_stmt_execute($stmt)) {
            echo '
            <script>
                alert("Flash Notice has been updated successfully");
    ...
    <section class="text-gray-600 body-font" id="joinUsSection">
        <div class="container mx-auto flex md:flex-row flex-col items-center">
            <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0">
                <div class="max-w-4xl mt-5 mb-3 p-6 mx-auto bg-indigo-600 rounded-md shadow-md dark:bg-gray-800 mt-0 mb-10 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <a href="#">
                        <h5 id="flash_subject_prev" class="mb-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                    ...
                    <div class="">
                        ...
                        <a href="#" class="mb-5 inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            Close
                        </a>
                    </div>
                </div>
            ...
            <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center text-center">
                <main>
                    <section class="max-w-4xl p-6 mx-auto bg-blue-600 rounded-md shadow-md dark:bg-gray-800 mt-10 mb-10">
                        <h1 class="text-xl font-bold text-white capitalize dark:text-white">Flash Notice</h1>
                        <form action="" method="POST" enctype="multipart/form-data">
                            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                                <div>
                                    <label class="text-white dark:text-gray-200">Trun Flash</label>
                                    <select name="trun_flash_notice" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                                        <option <?php if ($flash_notice['trun_flash'] == "1") echo 'selected'; ?> value="1">ON</option>
                                        <option <?php if ($flash_notice['trun_flash'] == "0") echo 'selected'; ?> value="0">OFF</option>
                                    </select>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-white">Banner Image</label>
                                        ...
                                        <div class="space-y-1 text-center">
                                            ...
                                            <div class="flex text-sm text-gray-600">
                                                <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                                                    ...
                                                    <input name="image_name" type="hidden" value="<?php echo $flash_notice['image_url']; ?>">
                                ...
                                <div>
                                    <label class="text-white dark:text-gray-200">Message <span style="color: red;">*</span></label>
                                    <textarea name="flash_message" id="flash_message" rows="4" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"><?php echo $flash_notice['message']; ?></textarea>
                                </div>
                            </div>
                            <div class="flex justify-end mt-6">
                                <button name="update_flash_notice" type="submit" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-pink-500 rounded-md hover:bg-pink-700 focus:outline-none focus:bg-gray-600">Save</button>
                            </div>
                        </form>
                    </section>
                </main>
...
<script>
    function displayFileName() {
        ...
        if (fileInput.files.length > 0) {
            ...
            fileInfoContainer.innerHTML = 'or drag and drop';
        }
    }
